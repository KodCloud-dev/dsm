...
wizard_vhost_setting_warning="系统在 Web Station 中检测到至少一台虚拟主机, 其根目录设置为“web”文件夹。系统更新之后, KodExplorer 的文件夹会从文件夹“web”移动到“web_packages”; 因此, 您将无法再通过此虚拟主机访问 KodExplorer。<br><br>如果您希望在更新之后使用相同域名，请进入 Web Station > 网页服务门户以删除虚拟主机。<br><br>请注意，KodExplorer 会在其文件夹移动之后保留其 http 群组的访问权限。如果用户需要访问 KodExplorer 的文件夹，请确保将其用户帐户添加到 http 群组。"
#!/bin/bash
